...
5. **每个结论都能回答"从哪来的"。** 数字必须挂到假设编号或证据来源上；没有数据的地方标"数据不足"。
...
按 `references/market-research.md` 与 `references/competitive-analysis.md` 执行研究，按 `references/business-model.md` 做商业模式与单位经济初判。纪律：
- 研究目标是**降低最大未知**，不是写报告；
